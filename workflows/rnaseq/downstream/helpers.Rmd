...
#' @param analysis.mode Type of analysis 'overrep' or 'gsea'
#'
...
#'
split.clusterProfiler.results <- function(x, analysis.mode){
    df <- x@result

    # set column name based on analysis.mode
    ...
    df.parse <- NULL
    for(k in 1:dim(df)[1]){
        g <- strsplit(as.character(df[k, id.col]), "/")[[1]]
...
#' Converts entrezID to symbol in enrichResult or gseaResult objects
#'
#' @param res clusterProfiler results. Can be enrichResult or gseaResult objects
#' @param genes.df Data frame with gene ID to symbol mapping. Must have SYMBOL
#'        and ENTREZID columns
#' @param id.col Column name with IDs to be converted. This is 'geneID' for
#'        enrichResult and 'core_enrichment' for gseaResult 
#'
#' @result enrichResult or gseaResult object with IDs converted to symbol
set.readable <- function(res, genes.df, id.col){
    id.vec <- genes.df[[comp]][[sel]]$SYMBOL
    names(id.vec) <- genes.df[[comp]][[sel]]$ENTREZID
    res.genes <- res@result[,id.col]
    for(j in 1:length(res.genes)){
        id.split <- strsplit(res.genes[j], "/")[[1]]
        temp <- paste(id.vec[id.split], collapse="/")
        res@result[j, id.col] <- temp
    }
    return(res)
}
...
#' @param label Label to use for the results. Will generate a filename
#' cprof.folder/label.txt and cprof.folder/label_split.txt
#' @param analysis.mode Type of analysis 'overrep' or 'gsea'
#'
#' @return List of the two files that are created on disk.
write.clusterprofiler.results <- function(res, cprof.folder, label, analysis.mode){
    ...
    res.split <- split.clusterProfiler.results(res, analysis.mode)
