...
#' Loads the orgdb for other functions
#' Copied from the DOSE package by Guangchuang Yu
#' @param OrgDb object or OrgDb name.
#'
#' @return orgdb object
...
#' Biological Id TRanslator copied from the clusterProfiler package by Guangchuang Yu
#' This version is altered so no error will be returned if a given key does not find a match
#' in the orgdb.
#' @param geneID input gene id
#' @param fromType input id type
#' @param toType output id type(s)
#' @param OrgDb annotation db
#' @param drop drop NA or not
#' 
#' @return data.frame mapiing input IDs with output IDs of the selected types
bitr_override <- function(geneID, fromType, toType, OrgDb, drop=TRUE) {
    ...
    # Create an empty vector the same as typical output
    # if the try fails, replace with an empty dataframe
    name_vec <- c(fromType, toType)
    tmp <- length(name_vec)
    res <- data.frame(matrix(ncol=tmp, nrow=0))
    colnames(res) <- name_vec
    try({res <- suppressWarnings(select(db,
                                   keys = geneID,
                                   keytype = fromType,
