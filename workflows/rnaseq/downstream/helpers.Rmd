...
#' Function to get AnnotationHub object
#'
#' @param species species name, e.g. 'Homo sapiens' or 'Mus musculus'
#' @param cache location of AnnotationHub cache.
#'
#' @return AnnotationHub object
get.ah <- function(species, cache){
...
#' Function to find AnnotationHub species accession for specified annotation db
#'
#' @param ah AnnotationHub object
#' @param species.name name of species, e.g. 'Homo sapiens' or 'Mus musculus'
#' @param db.type type of annotation db to use, e.g. 'OrgDb' or 'TxDb'
#'
#' @return AnnotationHub species accession key
find.annotationhub.name <- function(ah, species.name, db.type='OrgDb') {
    ah.query <- query(ah, db.type)
    ah.query.which <- grep(paste("^", species.name, "$", sep=""),
                           ah.query$species)

    # require at least one match
    stopifnot(length(ah.query.which) > 0)
    
    # warn of duplicate matches
    if (length(ah.query.which) > 1) {
       print("WARNING: found multiple candidate species in AnnotationHub: ");
       print(ah.query$species[ah.query.which])
    }

    # return first match
    return(names(ah.query)[ah.query.which[1]])
}

#' Function to perform GO enrichment, with optional parameters allowing parallel operation.
#'
#' @param gene gene list tested for enrichment
#' @param ont gene ontology tested for enrichment. Can be 'BP', 'CC' or 'MF'
#' @param keytype type of gene IDs
#' @param universe background list of genes to compare to the gene list
#' @param pvalueCutoff p-value threshold to determine statistical significance
#' @param qvalueCutoff FDR (q-value) threshold to determine statistical significance
#' @param orgdb OrgDb object from AnnotationHub (optional). Used only for non-parallel operation
#' @param species_accession OrgDb accession for species being tested. Used only for parallel operation.
#' @param cache location of AnnotationHub cache. Used only for parallel operation.
#'
#' @return clusterProfiler enrichResult object
enrichGO.fun <- function(gene,
                         ont,
                         keytype,
                         universe,
                         pvalueCutoff,
...
#' Function to perform KEGG/Reactome enrichment.
#'
#' @param gene gene list tested for enrichment
#' @param pathway type of pathway enrichment to perform. Can be 'KEGG' or 'Reactome'
#' @param organism abbreviation of species name. For example, if 'Homo sapiens', for KEGG enrichment it 
#'        should be 'hsa', for Reactome it should be 'human'.
#' @param keytype type of gene IDs. This is only used for KEGG and is usually 'ncbi-geneid'.
#' @param universe background list of genes to compare to the gene list
#' @param pvalueCutoff p-value threshold to determine statistical significance
#' @param qvalueCutoff FDR (q-value) threshold to determine statistical significance
#'
...
#'
#' @param res.list data.frame or DESeqResults with log2FoldChange and padj columns (or elements).
...
plotMA.label <- function(res,
  ...
  # set default alpha and lfc.thresh
  if(missing(alpha)) alpha <- 0.1
  if(missing(lfc.thresh)) lfc.thresh <- 0
  ...
  # get alpha and lfc thresh from metadata of DESeqResults object
  if(class(res) == 'DESeqResults'){
      alpha.i <- metadata(res)$alpha
      lfc.thresh.i <- metadata(res)$lfcThreshold
      if(alpha.i != alpha)
        warning('FDR threshold from metadata(res) does not match specified alpha')
      if(lfc.thresh.i != lfc.thresh)
        warning('lfc threshold from metadata(res) does not match specified lfc threshold')
  }
  res <- data.frame(res)
  
  # set alpha and lfc thresholds
  alpha <- ifelse(is.null(alpha), alpha.i, alpha)
  lfc.thresh <- ifelse(is.null(lfc.thresh), lfc.thresh.i, lfc.thresh)
  ...
  df <- df %>%
    mutate(significant = replace(significant,
                                 padj < alpha & !is.na(padj) & abs(log2FoldChange) >= fc.thres,
                                 'yes')) %>%
