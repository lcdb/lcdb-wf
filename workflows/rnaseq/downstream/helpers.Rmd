...
#'
#' @return OrgDb object
get.orgdb <- function(species, cache_choice, annotation_key_override=NA){
...
DESeqDataSetFromSalmon <- function (sampleTable, directory='.', design,
                                           ignoreRank=FALSE,  ...)
{
    txi <- tximport(sampleTable[, 'salmon.path'], type='salmon', txOut=TRUE, ignoreTxVersion=TRUE)
...
#'
#' @param deseq_obj_list A named list of lists. Each list should have two elements:
#'       1 should be a sampletable, 2 should be a design.
#' @param salmon.files Boolean for whether to use salmon; Files will automagically come from 
#'         the sample table if the incoming colData has been set up as required in DESeqDataSetFromSalmon.
...
#' 
make.dds.list <- function(deseq_obj_list, salmon.files=FALSE, 
                          combine.by=FALSE, remove.version=TRUE, parallel=FALSE){
    make.dds <- function(design_data, salmon.files, combine.by, remove.version)
    {
        ...
        subset.counts <- pluck(design_data, 'subset.counts')
        ...
        if(!salmon.files){
            ...
            dds <- DESeqDataSetFromSalmon(colData, design=design)
        }
