...
plotMA.label <- function(res,
  ...
  # evaluate whether res is a data.frame or DESeqResults object
  tryCatch(
    expr = {
      metadata(res)
      # get fdr.thresh
      if(is.null(fdr.thres)){
        fdr.thres <- metadata(res)[['alpha']]
      } else if(fdr.thres != metadata(res)[['alpha']]) {
        print(paste0('WARNING:',
              warning('Supplied fdr.thresh and DESeqResults object FDR threshold are different.')))
      }
      # get fc.thresh
      if(is.null(fc.thres)){
        fc.thres <- metadata(res)[['lfcThreshold']]
      } else if(fc.thres != metadata(res)[['lfcThreshold']]) {
        print(paste0('WARNING:',
                     warning('Supplied fc.thresh and DESeqResults object LFC threshold are different.')))
      }
...
#' @param lfc.threshold LFC threshold level at which to call significantly changing genes
#'
#' @return Dataframe of summarized results
my.summary <- function(res, dds, alpha, lfc.thresh, ...){
   if (missing(alpha)){
       alpha <- if (is.null(metadata(res)$alpha)){ 0.1 } else { metadata(res)$alpha }
   }
   if (missing(lfc.thresh)){
       lfc.thresh <- if (is.null(metadata(res)$lfcThreshold)){ 0 } else { metadata(res)$lfcThreshold }
   }
...
#'                 names: c('res', 'dds', 'label', 'alpha', 'lfc.thresh').
#'                 "res" is a DESeqResults object, "dds" is a DESeq object, 
#'                 "label" is a nicer-looking label to use, 'alpha' is the FDR
#'                 threshold and 'lfc.thresh' is the LFC threshold.
#'
#' @return Dataframe
summarize.res.list <- function(res.list){
    slist <- list()
    for (name in names(res.list)){
        lfc.thresh <- if (is.null(res.list[[name]]$lfc.thresh)) {
            if (is.null(metadata(res.list[[name]][['res']])$lfcThreshold)){ 0 } else { metadata(res.list[[name]][['res']])$lfcThreshold }
        } else { res.list[[name]]$lfc.thresh }
        x <- my.summary(res.list[[name]][['res']], res.list[[name]][['dds']],
                        metadata(res.list[[name]][['res']])$alpha, lfc.thresh)
...
get.sig <- function(x, direction='up', alpha=NULL, lfc.thresh=NULL, return.names=TRUE){
    tryCatch(
        # check whether x is a DESeq2 object or data.frame
        expr = {
            metadata(x)
            # get alpha
            if(is.null(alpha)){
                alpha <- metadata(x)[['alpha']]
            } else if(alpha != metadata(x)[['alpha']]) {
                print(paste0('WARNING:',
                            warning('Supplied alpha and DESeqResults object FDR threshold are different.')))
            }
            # get lfc.thresh
            if(is.null(lfc.thresh)){
                lfc.thresh <- metadata(x)[['lfcThreshold']]
            } else if(lfc.thresh != metadata(x)[['lfcThreshold']]) {
                print(paste0('WARNING:',
                             warning('Supplied lfc.thresh and DESeqResults object LFC threshold are different.')))
            }
        }
    )
    # if thresholds not supplied
    if (is.null(alpha)) {
        alpha <- 1
    }
    if (is.null(lfc.thresh)) {
