...
slurm_cores <- config$parallel$cores
if (config$parallel$parallel & slurm_cores != "") {
    cores <- as.numeric(slurm_cores)
} else {
    cores <- 1
}
```
...
# Check if we are running Overrepresentation Analysis (ORA) or Gene Set Enrichment Analysis (GSEA)
kind <- config$functional_enrichment$kind
if (is.null(kind)) {
  kind <- "ORA"
}

# We only need the universe_list/background gene pool if we're doing ORA
if (kind == "ORA" | kind == "OR") {
