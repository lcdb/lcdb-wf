...
types <- config$orgdb$orgdb_columns
```

```{r enrichall, results='hide', cache=TRUE, config=config$functional_enrichment, dependson='selections'}
...
genes.df <- nested.lapply(
    deg.list,
    function (x) {
        AnnotationDbi::select(
            orgdb, keys=x, columns=config$orgdb$orgdb_columns, keytype=config$orgdb$keytype)
    })
...
for(comp in names(genes.df)){
    ...
    for(sel in sel.names){
        ...
        gene <- genes.df[[comp]][[sel]][[config$functional_enrichment[['go_keytype']]]]
        ...
        kegg.res <- enrichKEGG(
            gene=gene,
            universe=config$functional_enrichment$path_uni, organism=config$functional_enrichment$kegg_organism,
        ...
        if (!is.null(kegg.res)){
            ...
            # in the case of KEGG
            if(config$functional_enrichment$id_convert){
...
# reasonable.
all.enrich <- lcdbwf::truncate_names(all.enrich, 50)
```
...
```{r plotgo, fig.width=15, fig.height=10, results='asis', cache=TRUE, dependson='enrichall'}
