...
```{r sample_heatmap, results='asis', cache=TRUE, config=config$plotting$covariates_for_plots, dependson='dds_initial'}
# Plot a clustered heatmap of sample distances, for QC.
mdcat(text$qc$clustered_heatmap)
...
#
# Example 1: 
control <- dds_coefs('main', group=='control')
treatment <- dds_coefs('main', group=='treatment')
contr_main <- make_results('main', 'LFC>0', contrast=treatment - control, type='ashr')
```

```{r results_02, dependson='dds_list', cache=TRUE}
contr_lfc1 <- make_results(
  dds="no_rep4",
