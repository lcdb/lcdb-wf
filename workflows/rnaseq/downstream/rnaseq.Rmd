...
# get annotation hub
ah <- get.ah(annotation_genus_species,
             cache=new.cache)

# get species accession
species_accession <- ifelse(is.na(annotation_key_override),
                            find.annotationhub.name(ah, annotation_genus_species),
...
#    set `nworkers` to the number of available cores and `parallel <- TRUE`.
#    Then, add the argument `parallel=TRUE` whenever you call DESeq()
nworkers <- 2
parallel <- FALSE
if(parallel) register(MulticoreParam(nworkers))
```
