...
#
#      {r, cache=TRUE, config=config$orgdb}
#
# Thereafter, any changes to config options under the "orgdb" section will then
# invalidate that chunk's cache -- along with any other chunks that also have
# config$orgdb included as a chunk option.
#
...
```{r salmon, cache=TRUE, eval=config$toggle$salmon}
...
```{r kallisto, cache=TRUE, eval=config$toggle$kallisto, cache.lazy=FALSE}
...
```{r salmonddstxi, cache=TRUE, eval=config$toggle$salmon}
...
```{r kallistoddstxi, cache=TRUE, eval=config$toggle$kallisto}
...
```{r dds_initial, cache=TRUE, config=config$main}
...
lcdbwf::plot.heatmap(vsd, colData, cols.for.grouping=config$plotting$covariates_for_plots)
```
...
### Size factors vs total read count
...
```{r assemble_variables, cache=TRUE, config=config$orgdb, dependson=knitr::all_labels()[grepl("^results_", knitr::all_labels())]}
...
```{r, results='asis'}
knitr::kable(summarize.res.list(res_list, config$main$alpha, config$main$lfc_thresh, dds_list))
```
...
```{r reportresults, results='asis', config=config$orgdb, dependson='assemble_variables', cache=TRUE}
...
for (name in names(res_list)){
  ...
  print(lcdbwf::plotMA.label(
    res_i,
    genes_to_label=genes_to_label,
    label_column=config$orgdb$label_column))

  lcdbwf::mdcat('### Volcano plot')
  print(lcdbwf::plot.volcano.label(
    res_i,
    genes_to_label=genes_to_label,
    label_column=config$orgdb$label_column))

  lcdbwf::mdcat('### Summary')
  print(knitr::kable(lcdbwf::my.summary(res_i, dds_i)))

  lcdbwf::mdcat('### P-value distribution')
  lcdbwf::pval.hist(res_i)
}
```
...
```{r upsetplots, results='asis', eval=length(res_list)>1, config=config$orgdb, dependson='selections'}
# Compare overlap across multiple differential expression contrasts.
lcdbwf::plot.upsets(res_list, label_column=config$orgdb$label_column)
```

```{r genepatterns, child='gene-patterns.Rmd', eval=config$toggle$gene_patterns}
...
```{r functionalenrichment, child='functional-enrichment.Rmd', eval=config$toggle$functional_enrichment}
