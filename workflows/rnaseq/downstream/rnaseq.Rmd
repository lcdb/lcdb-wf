...
```{r}
colData[, colnames(colData)[!colnames(colData) %in% exclude.for.printing]] %>%
    datatable()
```
...
dds_initial <- lcdbwf::make.dds(
    list(
         sampletable=colData,
         design=~group
     ),
    salmon.files=NULL,
    remove.version=strip.dotted.version,
    combine.by=NULL,
    parallel=parallel
...
dds.list <- map(lst, lcdbwf::make.dds,
                salmon.files=NULL,
                remove.version=strip.dotted.version,
                combine.by=NULL,
                parallel=parallel
...
# identify all variable names that start with "contr_" and store in a list
contrast_list <- ls()[grepl("^contr_", ls())]

# Convert the list of string variable names into a list of the objects
# themselves
res.list <- map(contrast_list, function(x) eval(parse(text=x)))

# Rename the items in the list to not have the "contr_" prefix
...
# Ensure all results objects have the same rownames
nms <- lapply(res.list, function (x) rownames(x[['res']]))
if (!all(sapply(nms, function(a) all(a == nms[[1]])))){
    stop("Gene IDs differ between results objects")}

# Here we look up the other information for genes in the OrgDb once and then
# attach that information to each results object
keytype <- 'ENSEMBL'
...
lookups <- list()
for (col in columns){
    lookups[[col]] <- mapIds(orgdb, keys=nms[[1]], column=col, keytype=keytype, multiVal='first')
}
...
for (name in names(res.list)){
    ...
    for (col in columns){
        res[[tolower(col)]] <- lookups[[col]]
    }
    for (col in columns){
        if (col %in% fill){
            lcol <- tolower(col)
            res[[lcol]] <- ifelse(is.na(res[[lcol]]), rownames(res), res[[lcol]])
        }
    }
    cn <- colnames(res)
    res$gene <- rownames(res)
    res <- res[, c('gene', cn)]
    res.list[[name]][['res']] <- res
}
```
...
```{r reportresults, results='asis', dependson=c('assemble_variables'), cache=TRUE}
...
for (name in names(res.list)){
  ...
  mdcat('### M-A plot and summary')


  # NOTE: gene labels --------------------------------------------------------
  #   By default we plot the symbols of the top 10 upregulated or downregulated
  #   genes, with the highest magnitude log2FoldChange. To change this behavior
  #   edit the following lines
  res.i.filtered <- res.i %>%
      as.data.frame() %>%
      arrange(log2FoldChange) %>%
      filter(!is.na(log2FoldChange))

  genes.to.label <- c(
        head(res.i.filtered, 5)[['symbol']],
        tail(res.i.filtered, 5)[['symbol']]
        )
  ...
  mdcat('### Volcano plot')
  print(plot.volcano.label(res.i, genes.to.label=genes.to.label, col='symbol'))

  mdcat('### P-value distribution')
  lcdbwf::pval.hist(res.i)
}
```

```{r selections, cache=TRUE, dependson='assemble_variables'}
