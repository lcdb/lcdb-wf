...
# Create lists to represent each future dds object. The third object in each 
# list is an optional list of arguments passed to the DESeqDataSet contructor.
# Ex. the subset.counts argument. Arguments to DESeq, such as parallel, should
# be passed to the make.dds.list function
#

#lst <- list(
#            main=list(colData, ~group, list(subset.counts=FALSE) ),
#            no.rep.2=list(colData %>% filter(replicate!=2), ~group, list(subset.counts=TRUE) ),
#            interaction=list(colData %>% filter(replicate!=2), 
#                             ~genotype + time + genotype:time, list(subset.counts=TRUE) )
#            )
#

lst <- list(
            main=list(colData, ~group),
            r = list(colData %>% filter(samplename!='sample4'), ~group, list(subset.counts=TRUE))
            )
dds.list <- make.dds.list(lst, salmon.files=txi, parallel=parallel, remove.version=strip.dotted.version)
```
