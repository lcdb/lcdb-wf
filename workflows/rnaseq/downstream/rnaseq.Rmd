...
```{r cutoffs, include=FALSE, cache=TRUE}
# NOTE: significance level---------------------------------------------------
# set the FDR threshold alpha
alpha <- 0.1
# set the log2 fold-change. An lfc.thresh of 2 is equivalent to a 4-fold change (log2(4)=2)
lfc.thresh <- 2
```
...
```{r results, cache=TRUE, dependson=c('cutoffs', 'dds_models')}
...
#       res.list[[contrast]][[lfc.thresh]] <- lfc threshold used for this contrast
#
...
res.list[['all']] <- list(
    res=lfcShrink(dds,
                  contrast=c('group', 'treatment', 'control'),
                  alpha=metadata(res.all)[['alpha']],
                  res=res.all,
                  type='normal'
                  ),
    dds=dds,
    label='Using a log2FoldChange threshold of 0',
    # lfcShrink does not record the res lfcThreshold in the metadata(),
    # indicate it here to use in subsequent plotting
    lfc.thresh=0
)
...
#    requiring genes to have >2^lfc.thresh-fold differences. Change to
#    reflect your experiment.
res.lfcthresh <- results(
    dds,
    contrast=c('group', 'treatment', 'control'),
    alpha=alpha,
    lfcThreshold=lfc.thresh)

res.list[['lfc_cutoff']] <- list(
    res=lfcShrink(
        dds,
        contrast=c('group', 'treatment', 'control'),
        alpha=metadata(res.lfcthresh)[['alpha']],
        res=res.lfcthresh,
        type='normal'
    ),
    dds=dds,
    label=paste0('Using a log2FoldChange threshold of >', lfc.thresh),
    # lfcShrink does not record the res lfcThreshold in the metadata(),
    # indicate it here to use in subsequent plotting
...
```{r, results='asis', dependson=c('cutoffs', 'attach')}
# Summarize all experiments
knitr::kable(summarize.res.list(res.list))
```
...
for (name in names(res.list)){
  ...
  lfc.thresh.i <- res.list[[name]][['lfc.thresh']]

  mdcat('## ', label, ' {.tabset}')
  mdcat('### Summary of results')
  print(knitr::kable(my.summary(res.i, dds.i, lfc.thresh=lfc.thresh.i)))
  ...
  mdcat('### M-A plot')
  print(plotMA.label(res.i,
                     fc.thres=lfc.thresh.i,
...
```{r selections, cache=TRUE, dependson=c('attach', 'cutoffs')}
sel.list <- list()
for (name in names(res.list)){
  ...
  sel.list[[name]] <- list(
    up=res[get.sig(res, lfc.thresh=lfc.thresh.i, direction='up'),],
    dn=res[get.sig(res, lfc.thresh=lfc.thresh.i, direction='dn'),]
    )
}
...
```{r child='gene-patterns.Rmd', eval=FALSE}
...
```{r child='functional-enrichment.Rmd', eval=FALSE}
