...
res.list[['all']] <- list(
    ...
    alpha=metadata(res.all)[['alpha']],
    lfc.thresh=metadata(res.all)[['lfcThreshold']]
)
...
res.list[['lfc_cutoff']] <- list(
    ...
    alpha=metadata(res.lfcthresh)[['alpha']],
    lfc.thresh=metadata(res.lfcthresh)[['lfcThreshold']]
)
```
...
for (name in names(res.list)){
  ...
  alpha.i <- res.list[[name]][['alpha']]
  lfc.thresh.i <- res.list[[name]][['lfc.thresh']]

  mdcat('## ', label, ' {.tabset}')
  mdcat('### Summary of results')
  print(knitr::kable(my.summary(res.i, dds.i, alpha.i, lfc.thresh=lfc.thresh.i)))
