...
#
#    This should be a three element vector, the name of a factor in the
#    design formula, name of the numerator for the fold change, and the
#    name of the denominator for the fold change. E.g.
#    > res <- results(dds, contrast=c('group', 'treatment', 'control')
#
# 2. using the 'name' parameter in the results() function call or 'coef'
#    in lfcShrink()
#
#    'name' should be one of the values returned by the resultsNames(dds)
#    denoting the precomputed results. E.g.
#    > resultsNames(dds)
#    [1] "Intercept"  "group_treatment_vs_control"
#    > res <- results(dds, name='group_treatment_vs_control')
#
# 3. a numeric contrast vector with one element for each element in
#    the resultsNames() function call. This is useful for arbitrary
#    comparisons in multi-factor designs with a grouping variable.
#
#    > resultsNames(dds)
#    [1] "Intercept"  "group_treatment_vs_control"
...
###  Most general case of specifying contrasts with numeric vector
#
#    If 'group' encodes all combinations of a two-factor experiment,
#    genotype: A, B, condition: I, II, group: IA, IB, IIA, IIB
#
#    Then you can make arbitrary comparisons by fitting an 'intercept-less'
#    model, e.g. design=~group + 0, and numeric contrast vectors.
#
...
#
#    2. interaction term: (IA vs IB) vs (IIA vs IIB)
