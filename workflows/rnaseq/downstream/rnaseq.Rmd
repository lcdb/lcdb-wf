...
Now we perform differential expression analysis for the contrasts listed in the table below.

Under the table are tabs, one for each contrast. Under each contrast's tab, there are more tabs for the various output.
...
**Interpretation notes:** A gene can only be found in one column in an UpSet plot. So if you want to
...
```{r}
lcdbwf::exported_excel(res_list, dds_list)
```
