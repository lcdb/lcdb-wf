...
which case all size factors would be 1.0. This is never the case; these size
factor estimates show how much sequencing depth varies across libraries.
...
# Assumption: you want the gene patterns to be plotted across "group" on the
# x-axis (this is the `time` argument to degPatterns). For more complicated
# designs, try the `col` argument to color by another factor.

# Set a hard limit -- beware plotting more genes than this as it could take a lot of
# time to perform the clustering.
lim <- 2000
ll <- lapply(res.list, get.sig, 'changed')
for (name in names(ll)){
    genes <- ll[[name]]

    # If there are fewer than the default `minc`, assume we want to plot them
    # anyway.
    minc <- min(length(genes) - 1, 15)
    ...
    d <- degPatterns(ma, colData.i, time='group', reduce=TRUE, cutoff=0.5, minc=minc)
